  ...
  return (
    ...
    <br/>
    {
      benefits.map((benefit:any, index:number)=>(
        <input type='text' key={index} name='Benefit' placeholder='You will be able to build a full stack LMS platform...'
        required className={`${styles.input} my-2`} value={benefit.title} onChange={(e)=>handleBenefitChange(index, e.target.value)}/>
      ))
    }
    ...
    <br/>
    {
      prerequisites.map((prerequisites:any, index:number)=>(
        <input type='text' key={index} name='prerequisite' placeholder='You need basic knowledge of JavaScript..'
        required className={`${styles.input} my-2`} value={prerequisites.title} onChange={(e)=>handlePrerequisitesChange(index, e.target.value)}/>
      ))
    }
