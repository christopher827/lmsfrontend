...
const CourseContent : FC <Props> =({courseContentData,setCourseContentData,active,setActive,handleSubmit:handleCourseSubmit}) =>{
  ...
  const [activeSection,setIsActiveSection] = useState(1)
...
setCourseContentData([...courseContentData,newContent])
  ...
  toast.error("Please fill all fields first")    
      ...
      setIsActiveSection(activeSection + 1)
      const newContent = {
        videoUrl : "",
        title:"",
        description:"",
...
<br/>
  ...
  <div className="w-full items-center justify-between my-0">
  {isCollapsed[index]?(
<>
{item.title  ? (
...
<input type='text' placeholder='sddr' className={`${styles.input}`} value={item.videoUrl} onChange={(e)=>{
  const updatedData = [...courseContentData];
...
<textarea cols={30} rows={8} placeholder='sddr'  className={`${styles.input} !h-min py-2`} value={item.description}
onChange={(e)=>{
  const updatedData = [...courseContentData];
...
<input type='text' placeholder='Source Code... (Link title)' className={`${styles.input} mt-6`} value={link.url} onChange={(e)=>{
    const updatedData = [...courseContentData];
